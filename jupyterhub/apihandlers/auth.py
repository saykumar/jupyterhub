...
class TokenAPIHandler(APIHandler):
    @token_authenticated
    def get(self, token):
        self.log.debug("[TokenApiHandler] Retrieving token: %s", token)
        orm_token = orm.APIToken.find(self.db, token)
        if orm_token is None:
            self.log.debug("[TokenApiHandler] API token not found. Checking OAuth access token")
            orm_token = orm.OAuthAccessToken.find(self.db, token)
        if orm_token is None:
            self.log.warning("[TokenApiHandler] No token found.")
            raise web.HTTPError(404)
        if orm_token.user:
            self.log.debug("[TokenApiHandler] User for token: %s", orm_token.user)
            model = self.user_model(self.users[orm_token.user])
            self.log.debug("[TokenApiHandler] User model: %s", model)
...
class OAuthHandler(BaseHandler, OAuth2Handler):
    ...
    def _dispatch_request(self):
        self.log.debug("[OAuthHandler] Provider: %s", self.provider)
